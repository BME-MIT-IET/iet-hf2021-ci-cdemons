...
class SortFail(Exception):
    ...
    def __init__(self, label, handles, color='k'):
        if label not in SortFail.labels:
            ...
            handles.append(Line([0],[0], marker=marker, label=label, color=color))
...
if __name__ == "__main__":
    ...
    lengths = [int(10 ** (x/3))  for x in range(10)]
    algs = [sort.merge_sort,
            sort.quick_sort,
            sort.bubble_sort,
         #   sort.radix_sort,
          #  sort.selection_sort,
           # sort.gnome_sort,
            sort.bogo_sort, #for the brave hearted
            #sort.bucket_sort,
            #sort.top_sort,
            sort.max_heap_sort
            ]
    labels = [alg.__name__ for alg in algs]
    colors = [cm.get_cmap('gist_rainbow')(i) for i in np.linspace(0.0, 1.0, len(labels))]
    handles = [Line([0],[0], color=color, lw=4, label=label) for label, color in zip(labels, colors)]
    ...
    for plot in range(5):
        ...
        axs[plot].grid()
        axs[plot].title.set_text(f'{"Value range: " if limit == 0 else ""}{_get_limit(limit)}-{_get_limit(limit+1)}')
